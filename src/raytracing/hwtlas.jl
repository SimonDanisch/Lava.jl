...
Every mutation above is addressed by `TLASHandle`, because that is how an HWTLAS
stores instances: one `InstanceBatch` per push!, each with its own GPU buffer.
`Raycore.TLAS` instead keeps one flat instance array and lets callers address a
range of it directly, and RayMakie's mesh path uses exactly that form — a plot
remembers `instance_idx = n_instances(accel)` from when it was pushed and later
writes one transform at that index.
...
crash but a scene that DID NOT MOVE: measured on an animated figure, two
opposite extremes of a walk cycle rendered 0 differing pixels out of 76800,
while the same scene on GLMakie differed in 3669. Ten plots, ten swallowed
errors, one perfectly still picture.

`n_instances` sums the batches in push order, so a flat index maps onto them by
walking that order. A range that does not line up with exactly one whole batch
is refused rather than approximated — `pending_updates` holds one transform
array per handle and has no way to express "part of a batch", and quietly
writing the wrong instances is the failure this method exists to end.
"""
function Raycore.update_instance_transforms!(hwtlas::HWTLAS, transforms::AbstractVector,
    ...
    for batch in hwtlas.instance_batches
        if i0 <= base + batch.n
            (i0 == base + 1 && n == batch.n) || error(
                ...
                "An HWTLAS updates whole batches; address it by handle instead.")
    ...
    error("update_instance_transforms!: instance index $(i0) past the end " *
          "($(base) instances in $(length(hwtlas.instance_batches)) batches).")
end

Raycore.update_instance_transforms!(hwtlas::HWTLAS, transforms::AbstractVector,
                                    n_to_update::Integer) =
...
Raycore.update_transforms!(hwtlas::HWTLAS, handle::Raycore.TLASHandle,
