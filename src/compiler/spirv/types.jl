    ...
    for fn in LLVM.functions(mod)
        for bb in LLVM.blocks(fn)
            for inst in LLVM.instructions(bb)
                _collect_pointee_types!(ptm, inst)
            end
        end
    end
...
function _collect_pointee_types!(ptm::PointeeTypeMap, inst::LLVM.Instruction)
    ...
    elseif inst isa LLVM.IntToPtrInst
        ...
        _collect_inttoptr!(ptm, inst)
    elseif inst isa LLVM.AtomicRMWInst
        _collect_atomicrmw!(ptm, inst)
    elseif inst isa LLVM.AtomicCmpXchgInst
        _collect_cmpxchg!(ptm, inst)
    end
end

function _collect_alloca!(ptm::PointeeTypeMap, inst::LLVM.AllocaInst)
...
function _collect_load!(ptm::PointeeTypeMap, inst::LLVM.LoadInst)
    ...
    if loaded_type isa LLVM.PointerType
        _infer_loaded_ptr_pointee!(ptm, inst)
    end
end

"""Infer the pointee type of a loaded pointer value by tracing its users."""
function _infer_loaded_ptr_pointee!(ptm::PointeeTypeMap, load_inst::LLVM.LoadInst)
    # First try direct users (GEP, load, store-as-target, atomicrmw, cmpxchg)
    pointee = _infer_pointee_from_users(load_inst)
    if pointee !== nothing
    ...
    for use in LLVM.uses(load_inst)
        ...
        if user isa LLVM.StoreInst && LLVM.operands(user)[1] === load_inst
            ...
            pointee = _infer_type_through_alloca(store_target, load_inst)
            if pointee !== nothing
...
function _infer_type_through_alloca(store_target::LLVM.Value, original_ptr::LLVM.Value)
    ...
    if store_target isa LLVM.GetElementPtrInst
        base, offset = _walk_gep_to_base(store_target)
        if base !== nothing && offset !== nothing
            result = _find_ptr_load_at_offset(base, offset)
            result !== nothing && return result
        end
    end
...
function _walk_gep_to_base(gep::LLVM.GetElementPtrInst)
    total_offset = 0
    current = gep
    ...
    for _ in 1:10
        ...
        offset = _compute_gep_byte_offset(src_ty, indices)
...
function _compute_gep_byte_offset(src_ty::LLVM.LLVMType, indices::Vector{<:LLVM.Value})
    offset = 0
    current_ty = src_ty
...
function _approx_sizeof(ty::LLVM.LLVMType)
    ...
    elseif ty isa LLVM.ArrayType
        n = LLVM.length(ty)
        elem_sz = _approx_sizeof(LLVM.eltype(ty))
...
function _find_ptr_load_at_offset(alloca::LLVM.AllocaInst, target_offset::Int)
    # Collect all GEPs and direct accesses from this alloca
    for use in LLVM.uses(alloca)
        user = LLVM.user(use)
        result = _check_gep_or_load_at_offset(user, alloca, target_offset, 0)
        result !== nothing && return result
    end
    return nothing
end
...
function _check_gep_or_load_at_offset(user::LLVM.Value, base::LLVM.Value, target_offset::Int, current_offset::Int)
    ...
    elseif user isa LLVM.GetElementPtrInst && LLVM.operands(user)[1] === base
        ...
        gep_offset = _compute_gep_byte_offset(src_ty, indices)
        gep_offset === nothing && return nothing
        new_offset = current_offset + gep_offset

        for use2 in LLVM.uses(user)
            user2 = LLVM.user(use2)
            result = _check_gep_or_load_at_offset(user2, user, target_offset, new_offset)
            result !== nothing && return result
        end
    end
    return nothing
...
function _infer_pointee_from_users(ptr_value::LLVM.Value)
    _infer_pointee_from_users(ptr_value, Set{LLVM.Value}())
end
...
function _collect_store!(ptm::PointeeTypeMap, inst::LLVM.StoreInst)
...
function _collect_gep!(ptm::PointeeTypeMap, inst::LLVM.GetElementPtrInst)
...
function _collect_inttoptr!(ptm::PointeeTypeMap, inst::LLVM.IntToPtrInst)
    ...
    for use in LLVM.uses(inst)
        ...
        if user isa LLVM.GetElementPtrInst
            ...
            if src_ty isa LLVM.IntegerType && LLVM.width(src_ty) == 8
                ...
                pointee = _infer_type_from_gep_users(user)
                if pointee !== nothing
...
function _infer_type_from_gep_users(gep::LLVM.GetElementPtrInst)
...
function _collect_call!(ptm::PointeeTypeMap, inst::LLVM.CallInst)
...
function _collect_bitcast!(ptm::PointeeTypeMap, inst::LLVM.BitCastInst)
...
function _collect_atomicrmw!(ptm::PointeeTypeMap, inst::LLVM.AtomicRMWInst)
...
function _collect_cmpxchg!(ptm::PointeeTypeMap, inst::LLVM.AtomicCmpXchgInst)
...
"""
    _compute_gep_result_type(source_ty, gep_inst) -> Union{LLVMType, Nothing}
...
"""
function _compute_gep_result_type(source_ty::LLVM.LLVMType, inst::LLVM.GetElementPtrInst)
    ops = LLVM.operands(inst)
    _compute_gep_result_type_from_ops(source_ty, ops)
end

function _compute_gep_result_type(source_ty::LLVM.LLVMType, val::LLVM.ConstantExpr)
    ops = LLVM.operands(val)
    _compute_gep_result_type_from_ops(source_ty, ops)
end

function _compute_gep_result_type_from_ops(source_ty::LLVM.LLVMType, ops)
    ...
    for i in 3:length(ops)  # skip base ptr and first index
        idx = ops[i]
        current_ty = _index_into_type(current_ty, idx)
...
"""
    _index_into_type(ty, idx) -> Union{LLVMType, Nothing}
...
"""
function _index_into_type(ty::LLVM.LLVMType, idx::LLVM.Value)
...
function map_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    ...
    spirv_id = _emit_llvm_type!(ctx, ty)
...
function map_workgroup_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    ...
    spirv_id = _emit_workgroup_type!(ctx, ty)
    ctx.workgroup_type_map[ty] = spirv_id
    return spirv_id
end

function _emit_workgroup_type!(ctx::SPIRVTypeContext, ty::LLVM.ArrayType)
...
function _emit_workgroup_type!(ctx::SPIRVTypeContext, ty::LLVM.StructType)
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.IntegerType)
    w = LLVM.width(ty)
    if w == 1
        # i1 → OpTypeBool
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMHalf)
    return emit_type_float!(ctx.mod, UInt32(16))
end

function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMFloat)
    return emit_type_float!(ctx.mod, UInt32(32))
end

function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMDouble)
    return emit_type_float!(ctx.mod, UInt32(64))
end

function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.FloatingPointType)
    error("Unsupported floating point type: $(typeof(ty))")
end

function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.ArrayType)
    # [N x T] → OpTypeArray
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.StructType)
    # { T1, T2, ... } → OpTypeStruct
    ...
    for (i, mt) in enumerate(member_types)
        if mt isa LLVM.PointerType
            push!(member_spirv_ids, _map_struct_ptr_member!(ctx, ty, i - 1, mt))
        else
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.VectorType)
    # <N x T> → OpTypeVector
    elem_spirv = map_type!(ctx, eltype(ty))
    n = length(ty)
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.PointerType)
...
"""
    _map_struct_ptr_member!(ctx, struct_ty, member_idx, ptr_ty) -> UInt32
...
"""
function _map_struct_ptr_member!(ctx::SPIRVTypeContext, struct_ty::LLVM.StructType,
...
"""
function _scan_load_from_struct_base!(ctx::SPIRVTypeContext, load_inst::LLVM.LoadInst)
...
"""
function _find_offset0_ptr_member(ty::LLVM.LLVMType)
    current = ty
    while true
...
function _scan_gep_for_struct_ptr_member!(ctx::SPIRVTypeContext, gep::LLVM.GetElementPtrInst)
    ...
    _trace_gep_ptr_users!(ctx, gep, key, as)
end
...
"""
function _trace_gep_ptr_users!(ctx::SPIRVTypeContext, gep::LLVM.Value,
        ...
        if user isa LLVM.LoadInst
            ...
            for load_use in LLVM.uses(user)
                ...
                if load_user isa LLVM.GetElementPtrInst
                    ...
                    if sub_src isa LLVM.IntegerType && LLVM.width(sub_src) == 8
                        # Byte-offset GEP — trace through to find real type
                        pointee = _infer_type_from_gep_users(load_user)
                        if pointee !== nothing
                            ctx.struct_ptr_members[key] = (pointee, as)
                            return
                        end
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.FunctionType)
...
function _emit_llvm_type!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
...
"""
function _infer_ptr_type_from_source_gep(ctx::SPIRVTypeContext, ptr_value::LLVM.Value)
    ...
    return _find_ptr_member_type_in_hierarchy(ctx, src_pointee)
end
...
function _find_ptr_member_type_in_hierarchy(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    if ty isa LLVM.ArrayType
        return _find_ptr_member_type_in_hierarchy(ctx, LLVM.eltype(ty))
    elseif ty isa LLVM.StructType
        ...
        for ft in LLVM.elements(ty)
            if ft isa LLVM.StructType || ft isa LLVM.ArrayType
                result = _find_ptr_member_type_in_hierarchy(ctx, ft)
                result !== nothing && return result
            end
        end
    end
...
function map_constant!(ctx::SPIRVTypeContext, val::LLVM.Constant)
    ...
    if val isa LLVM.ConstantInt
        return _map_constant_int!(ctx, val, ty)
    elseif val isa LLVM.ConstantFP
        return _map_constant_fp!(ctx, val, ty)
...
function _map_constant_int!(ctx::SPIRVTypeContext, val::LLVM.ConstantInt, ty::LLVM.IntegerType)
    w = LLVM.width(ty)
...
function _map_constant_fp!(ctx::SPIRVTypeContext, val::LLVM.ConstantFP, ty::LLVM.LLVMFloat)
    fval = convert(Float32, val)
    return emit_constant_f32!(ctx.mod, fval)
end

function _map_constant_fp!(ctx::SPIRVTypeContext, val::LLVM.ConstantFP, ty::LLVM.LLVMDouble)
    type_id = map_type!(ctx, ty)
...
function _map_constant_fp!(ctx::SPIRVTypeContext, val::LLVM.ConstantFP, ty::LLVM.LLVMHalf)
    type_id = map_type!(ctx, ty)
...
function _map_constant_fp!(ctx::SPIRVTypeContext, val::LLVM.ConstantFP, ty::LLVM.FloatingPointType)
    error("Unsupported float constant type: $(typeof(ty))")
end

function _map_undef!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    type_id = map_type!(ctx, ty)
    # OpUndef (opcode 1)
    id = fresh_id!(ctx.mod)
...
function _map_null_constant!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    type_id = map_type!(ctx, ty)
...
function decorate_psb_struct_layouts!(ctx::SPIRVTypeContext, llvm_mod::LLVM.Module)
    ...
    for (ptr_val, (pointee_ty, _priority)) in ctx.ptm.map
        _is_psb_value(ptr_val) || continue
        ...
        _collect_nested_types_for_psb!(all_psb_struct_types, all_psb_array_types, pointee_ty)
    end
    ...
    for llvm_sty in all_psb_struct_types
        _collect_nested_member_structs!(nested_structs, llvm_sty)
    end
...
function _is_psb_value(val::LLVM.Value)
    ty = LLVM.value_type(val)
...
function _collect_nested_member_structs!(nested::Set{LLVM.StructType}, sty::LLVM.StructType)
    for elem in LLVM.elements(sty)
        _collect_nested_member_structs_inner!(nested, elem)
    end
end

function _collect_nested_member_structs_inner!(nested::Set{LLVM.StructType}, ty::LLVM.LLVMType)
    if ty isa LLVM.StructType
        ty in nested && return
        push!(nested, ty)
        for elem in LLVM.elements(ty)
    ...
    elseif ty isa LLVM.ArrayType
        _collect_nested_member_structs_inner!(nested, eltype(ty))
    end
end

"""Recursively collect all struct and array types within a type (for PSB decoration)."""
function _collect_nested_types_for_psb!(structs::Set{LLVM.StructType},
    ...
    if ty isa LLVM.StructType
        ...
        push!(structs, ty)
        for elem in LLVM.elements(ty)
            _collect_nested_types_for_psb!(structs, arrays, elem)
        end
    elseif ty isa LLVM.ArrayType
        if !(ty in arrays)
            push!(arrays, ty)
            _collect_nested_types_for_psb!(structs, arrays, eltype(ty))
        end
    end
end

"""Compute byte size of an LLVM type using DataLayout for accurate struct sizes."""
function _compute_type_size_with_dl(ty::LLVM.LLVMType, dl::LLVM.DataLayout)
    if ty isa LLVM.StructType
        ...
        return UInt32(last_offset + _compute_type_size_with_dl(last_elem, dl))
    elseif ty isa LLVM.ArrayType
        return UInt32(length(ty)) * _compute_type_size_with_dl(eltype(ty), dl)
...
function collect_module_types!(ctx::SPIRVTypeContext, llvm_mod::LLVM.Module)
    ...
    for fn in LLVM.functions(llvm_mod)
        ...
        for param in LLVM.parameters(fn)
            _collect_value_type!(ctx, param)
        end

        # Instruction result types
        for bb in LLVM.blocks(fn)
            for inst in LLVM.instructions(bb)
                _collect_value_type!(ctx, inst)
            end
        end
    end
end

function _collect_value_type!(ctx::SPIRVTypeContext, val::LLVM.Value)
    ty = LLVM.value_type(val)
    if ty isa LLVM.PointerType
        # Pointer type — don't map directly, handled per-use via PointeeTypeMap
        return
    end
    _collect_type_recursive!(ctx, ty)
end

function _collect_type_recursive!(ctx::SPIRVTypeContext, ty::LLVM.LLVMType)
    if ty isa LLVM.PointerType
