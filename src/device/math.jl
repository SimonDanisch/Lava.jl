...
# which the SPIR-V emitter cannot answer as `FAbs(x) * FSign(y)`: GLSL.std.450
# FSign(0) == 0, so that produces 0 for any zero y and silently breaks
# ray-tracing `safe_invdir` clamps and other IEEE-dependent code.
